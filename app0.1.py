...
def send_email_notification(user_name, mobile, scheduled_date):
    ...
    msg['To'] = user_name # Change to your recipient

    try:
        with smtplib.SMTP(cfg.MAIL_SERVER, cfg.MAIL_PORT) as s:
            s.starttls() # If required by your SMTP server
            # s.login(MAIL_USER, MAIL_PASS) # If authentication is needed
            s.send_message(msg)
...
# Atexit is used to ensure the sessions are closed properly
#atexit.register(lambda: sbc_client.close() if sbc_client else None)
...
def manage_users():
    print("Received request for /audssoncall/api/users") # Add this line
    """API endpoint to get all users or add a new user."""
    # conn = get_db_connection()
    # cursor = conn.cursor()
    
    # if request.method == 'GET':
    #     cursor.execute("SELECT id, name, mobile FROM OnCallUsers ORDER BY name")
    #     users = [{'id': row.id, 'name': row.name, 'mobile': row.mobile} for row in cursor.fetchall()]
    ...
    if request.method == 'GET':
        cursor.execute("SELECT id, name, mobile FROM OnCallUsers ORDER BY name")
        # pyodbc.Row is not a thing, fetchall() returns a list of tuples
...
def update_user(user_id):
    # """API endpoint to update a user's mobile number."""
    # data = request.get_json()
    ...
    cursor = conn.cursor()
    sql = "UPDATE OnCallUsers SET mobile = ? WHERE id = ?"
    cursor.execute(sql, (data['mobile'], user_id)) # Corrected tuple passing
...
def manage_oncall():
    ...
    """API endpoint to get current on-call numbers or update them."""
    # if request.method == 'GET':
    #     # Corrected call to only use the keyword argument for 'action'
    #     statuses = sbc_client.sbc_interaction(action="check")
    #     return jsonify(statuses)
    
    # if request.method == 'POST':
    #     data = request.get_json()
    #     # ... (rest of your POST logic is already correct)
...
def manage_schedules():
    ...
    #             'scheduled_datetime': row.scheduled_datetime.isoformat(), 'status': row.status
    ...
    #     scheduled_datetime = data.get('scheduled_datetime') # Expecting ISO format e.g., "YYYY-MM-DDTHH:MM"

    #     if not all([user_id, scheduled_datetime]):
    ...
    #     send_email_notification(user.name, user.mobile, scheduled_datetime)
...
def update_oncall_api():
    ...
    try:
        ...
        # Call your refactored sbc_interaction function with the 'update' action
        results = sbc_client.sbc_interaction(action='update', mobile=mobile_number)

        # Check if all updates were successful and return a combined status
...
def delete_user(user_id):
    ...
    try:
        # First, delete scheduled jobs for the user to prevent foreign key issues
...
if __name__ == '__main__':
    ...
    scheduler.add_job(run_scheduled_updates, 'interval', minutes=5) # Check for jobs every 5 minutes
    scheduler.start()
    
    # Note: For production on IIS, this __main__ block won't run. 
    # The scheduler needs to be run as a separate process or Windows Service.
    app.run(debug=True)